---
title: "FAQ"
description: "Frequently asked questions about Bot setup, configuration, and usage"
---
...
- [Quick start and first-run setup](#quick-start-and-firstrun-setup)
  ...
  - [What’s the recommended way to install and set up Bot?](#whats-the-recommended-way-to-install-and-set-up-bot)
  ...
  - [I can't access docs.hanzo.bot (SSL error). What now?](#i-cant-access-docsbot-ssl-error-what-now)
  - [What’s the difference between stable and beta?](#whats-the-difference-between-stable-and-beta)
- [How do I install the beta version, and what’s the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  - [How do I try the latest bits?](#how-do-i-try-the-latest-bits)
  - [How long does install and onboarding usually take?](#how-long-does-install-and-onboarding-usually-take)
  - [Installer stuck? How do I get more feedback?](#installer-stuck-how-do-i-get-more-feedback)
  - [Windows install says git not found or bot not recognized](#windows-install-says-git-not-found-or-bot-not-recognized)
  - [The docs didn’t answer my question - how do I get a better answer?](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [How do I install Bot on Linux?](#how-do-i-install-bot-on-linux)
  - [How do I install Bot on a VPS?](#how-do-i-install-bot-on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask Bot to update itself?](#can-i-ask-bot-to-update-itself)
  ...
  - [If I buy a Mac mini to run Bot, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-bot-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different Bots?](#can-multiple-people-use-one-whatsapp-number-with-different-bots)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What’s the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run Bot on a dedicated machine?](#how-important-is-it-to-run-bot-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run Bot in a VM and what are the requirements](#can-i-run-bot-in-a-vm-and-what-are-the-requirements)
- [What is Bot?](#what-is-bot)
  - [What is Bot, in one paragraph?](#what-is-bot-in-one-paragraph)
  - [What’s the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for Bot](#what-are-the-top-five-everyday-use-cases-for-bot)
  - [Can Bot help with lead gen outreach ads and blogs for a SaaS](#can-bot-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can Bot run tasks on a schedule or continuously in the background?](#can-bot-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with Bot saved locally?](#is-all-data-used-with-bot-saved-locally)
  - [Where does Bot store its data?](#where-does-bot-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What’s the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall Bot?](#how-do-i-completely-uninstall-bot)
...
- [Config basics](#config-basics)
  ...
  - [Can the Bot browser run headless?](#can-the-bot-browser-run-headless)
...
- [Remote gateways + nodes](#remote-gateways-nodes)
  ...
  - [Can two Bots talk to each other (local + VPS)?](#can-two-bots-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does Bot load environment variables?](#how-does-bot-load-environment-variables)
...
- [Sessions & multiple chats](#sessions-multiple-chats)
  ...
  - [Is there a way to make a team of Bots one CEO and many agents](#is-there-a-way-to-make-a-team-of-bots-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset Bot but keep it installed?](#how-do-i-completely-reset-bot-but-keep-it-installed)
  ...
  - [Why doesn’t Bot reply in a group?](#why-doesnt-bot-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What models do the default agents use?](#what-models-do-the-default-agents-use)
...
- [Gateway: ports, “already running”, and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `bot gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-bot-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `bot gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-bot-gateway-status-show-config-cli-and-config-service-different)
  - [What does “another gateway instance is already listening” mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run Bot in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-bot-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart Bot?](#i-closed-my-terminal-on-windows-how-do-i-restart-bot)
  ...
  - [ELI5: `bot gateway restart` vs `bot gateway`](#eli5-bot-gateway-restart-vs-bot-gateway)
  - [What’s the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media & attachments](#media-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose Bot to inbound DMs?](#is-it-safe-to-expose-bot-to-inbound-dms)
...
5) **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/bot/bot-*.log | head -1)"
   ```
...
```bash
curl -fsSL https://hanzo.bot/install.sh | bash -s -- --install-method git
```
...
https://github.com/hanzoai/bot/issues
https://github.com/hanzoai/bot/pulls
...
What they do:
- `bot status`: quick snapshot of gateway/agent health + basic config.
- `bot models status`: checks provider auth + model availability.
- `bot doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `bot status --all`, `bot logs --follow`,
`bot gateway status`, `bot health --verbose`.

Quick debug loop: [First 60 seconds if something's broken](#first-60-seconds-if-somethings-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### Whats the recommended way to install and set up Bot
...
If you don’t have a global install yet, run it via `pnpm bot onboard`.
...
1) Install Bot on the new machine.
2) Copy `$BOT_STATE_DIR` (default: `~/.bot`) from the old machine.
3) Copy your workspace (default: `~/bot`).
4) Run `bot doctor` and restart the Gateway service.
...
Related: [Where things live on disk](/help/faq#where-does-bot-store-its-data),
...
https://github.com/hanzoai/bot/blob/main/CHANGELOG.md

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### I cant access docsbot SSL error What now
...
detail: [Troubleshooting](/help/troubleshooting#docsbot-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: https://spa.xfinity.com/check_url_status.

If you still can't reach the site, the docs are mirrored on GitHub:
https://github.com/hanzoai/bot/tree/main/docs
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://hanzo.bot/install.sh | bash -s -- --beta
```
...
```bash
curl -fsSL https://hanzo.bot/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://hanzo.bot/install.sh | bash -s -- --beta --verbose
```
...
### Windows install says git not found or bot not recognized
...
**2) bot is not recognized after install**
...
### Can I ask Bot to update itself
...
bot update
bot update status
bot update --channel stable|beta|dev
bot update --tag <dist-tag|version>
bot update --no-restart
```

If you must automate from an agent:

```bash
bot update --yes --no-restart
bot gateway restart
```
...
Bot supports **OpenAI Code (Codex)** via OAuth or by reusing your Codex CLI login (`~/.codex/auth.json`). The wizard can import the CLI login or run the OAuth flow and will set the default model to `openai-codex/gpt-5.2` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).
...
Steps:
1) Enable the plugin: `bot plugins enable google-gemini-cli-auth`
2) Login: `bot models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. Bot needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
### If I buy a Mac mini to run Bot can I connect it to my MacBook Pro
...
- Use `bot nodes status` / `bot nodes list` to see it.
...
- DM your bot, then run `bot logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different Bots
...
**Bot-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you’re actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run Bot on a dedicated machine
...
Bot is lightweight. For a basic Gateway + one chat channel:
...
### Can I run Bot in a VM and what are the requirements
...
### What are the top five everyday use cases for Bot
...
### Can Bot help with lead gen outreach ads and blogs for a SaaS
...
Showcase: https://hanzo.bot/showcase
...
Use **Skills** (CLI) or drop skills into your workspace. The macOS Skills UI isn’t available on Linux.
Browse skills at https://skills.com.

Install the Skills CLI (pick one package manager):
...
### Can Bot run tasks on a schedule or continuously in the background
...
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-bot-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.
...
3) Override the skill metadata (workspace or `~/.bot/skills`) to allow Linux:
   ...
   metadata: {"bot":{"os":["darwin","linux"],"requires":{"bins":["imsg"]}}}
   ---
   ```
4) Start a new session so the skills snapshot refreshes.

For iMessage specifically, you can also point `channels.imessage.cliPath` at an SSH wrapper (Bot only needs stdio). See [iMessage](/channels/imessage).
...
Skills installs into `./skills` under your current directory (or falls back to your configured Bot workspace); Bot treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.bot/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [Skills](/tools/skills).
...
### Is all data used with Bot saved locally
...
| `$BOT_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth + API keys) |
| `$BOT_STATE_DIR/agents/<agentId>/agent/auth.json` | Runtime auth cache (managed automatically) |
...
Legacy single‑agent path: `~/.bot/agent/*` (migrated by `bot doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/bot`).
...
### How do I completely uninstall Bot
...
Bot reads an optional **JSON5** config from `$BOT_CONFIG_PATH` (default: `~/.bot/bot.json`):

```
$BOT_CONFIG_PATH
```

If the file is missing, it uses safe‑ish defaults (including a default workspace of `~/bot`).
...
### Can the Bot browser run headless
...
Check the basics:
- Gateway is running: `bot gateway status`
- Gateway health: `bot status`
- Channel health: `bot channels status`
...
### Can two Bots talk to each other local VPS
...
- If you have no backup, re-run `bot doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:
- Use `bot config set` for small changes.
- Use `bot configure` for interactive edits.
...
### How does Bot load environment variables
...
### Is there a way to make a team of Bots one CEO and many agents
...
### How do I completely reset Bot but keep it installed
...
- Dev reset: `bot gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
### Why doesnt Bot reply in a group
...
- Use `bot doctor` to spot stray workspaces and profile mismatches.
...
Bot’s default model is whatever you set as:
...
### What models do the default agents use

- **Primary agent:** Anthropic Opus (`anthropic/claude-opus-4-5`) — see [Anthropic](/providers/anthropic).
- **Lightweight agent:** MiniMax M2.1 (`minimax/MiniMax-M2.1`) — see [MiniMax](/providers/minimax).
...
Yes. Bot ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
Cooldowns apply to failing profiles (exponential backoff), so Bot can keep responding even when a provider is rate‑limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  ...
  - Legacy: `~/.bot/agent/*` (migrated by `bot doctor`)
...
Bot uses provider‑prefixed IDs like:
...
bot models auth order get --provider anthropic

# Lock rotation to a single profile (only try this one)
bot models auth order set --provider anthropic anthropic:claude-cli

# Or set an explicit order (fallback within provider)
bot models auth order set --provider anthropic anthropic:claude-cli anthropic:default
...
### Why does bot gateway status say Runtime running but RPC probe failed
...
### Why does bot gateway status show Config cli and Config service different
...
### How do I run Bot in remote mode client connects to a Gateway elsewhere
...
Notes:
- `bot gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
Facts (from code):
- The Control UI stores the token in browser localStorage key `bot.control.settings.v1`.
- The UI can import `?token=...` (and/or `?password=...`) once, then strips it from the URL.

Fix:
- Fastest: `bot dashboard` (prints + copies tokenized link, tries to open; shows SSH hint if headless).
- If you don’t have a token yet: `bot doctor --generate-gateway-token`.
...
- Still stuck? Run `bot status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
### I closed my terminal on Windows how do I restart Bot
...
1) Is the Gateway running? `bot gateway status`
2) Is the Gateway healthy? `bot status`
3) Does the UI have the right token? `bot dashboard`
...
### ELI5 bot gateway restart vs bot gateway

- `bot gateway restart`: restarts the **background service** (launchd/systemd).
- `bot gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `bot gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Bot assistant setup](/start/bot) and [Agent send](/tools/agent-send).
...
### Is it safe to expose Bot to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM‑capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `bot pairing approve <channel> <code>`
  - Pending requests are capped at **3 per channel**; check `bot pairing list <channel>` if a code didn’t arrive.
...
Bot blocks **cross‑provider** messaging by default. If a tool call is bound
...
Still stuck? Ask in [Discord](https://discord.com/invite/bot) or open a [GitHub discussion](https://github.com/hanzoai/bot/discussions).
