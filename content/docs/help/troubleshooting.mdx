...
You can also set `BOT_VERBOSE=1` instead of the flag.
...
### `docs.hanzo.bot` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.hanzo.bot` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.hanzo.bot` to the allowlist, then retry.
...
- Check the allowlist: `bot config get agents.defaults.models`
...
If you can, include the relevant log tail from `bot logs --follow`.
