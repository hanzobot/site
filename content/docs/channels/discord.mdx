...
description: "Discord bot support status, capabilities, and configuration"
---
...
## Goals
- Talk to Bot via Discord DMs or guild channels.
...
3. Configure Bot with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `bot pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running Bot in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents Bot needs
...
Discord uses numeric ids everywhere; Bot config prefers ids.
...
### 6) Verify it works
1. Start the gateway.
2. In your server channel, send: `@Bot hello` (or whatever your bot name is).
3. If nothing happens: check **Troubleshooting** below.

### Troubleshooting
- First: run `bot doctor` and `bot channels status --probe` (actionable warnings + quick audits).
...
- On startup, Bot resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:
- The registered commands mirror Bot’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; Bot enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`bot gateway --force`) after confirming no other processes own the Discord session.
