...
Nostr is a decentralized protocol for social networking. This channel enables Bot to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`bot onboard`) and `bot channels add` list optional channel plugins.
...
```bash
bot plugins install @bot/nostr
```

Use a local checkout (dev workflows):

```bash
bot plugins install --link <path-to-bot>/extensions/nostr
```
