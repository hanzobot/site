---
title: "iMessage (imsg)"
description: "iMessage support via imsg (JSON-RPC over stdio), setup, and chat_id routing"
---
...
3) Configure Bot with `channels.imessage.cliPath` and `channels.imessage.dbPath`.
...
- Full Disk Access for Bot + `imsg` (Messages DB access).
...
2) Create a macOS user (example: `botshome`) and sign into it.
...
If you want iMessage on another Mac, set `channels.imessage.cliPath` to a wrapper that runs `imsg` on the remote macOS host over SSH. Bot only needs stdio.
...
**Remote attachments:** When `cliPath` points to a remote host via SSH, attachment paths in the Messages database reference files on the remote machine. Bot can automatically fetch these over SCP by setting `channels.imessage.remoteHost`:
...
If `remoteHost` is not set, Bot attempts to auto-detect it by parsing the SSH command in your wrapper script. Explicit configuration is recommended for reliability.
...
│ - bot gateway           │          SCP (attachments)        │ - Messages signed in     │
...
Example wrapper (`~/.bot/scripts/imsg-ssh`):
```bash
#!/usr/bin/env bash
...
Multi-account support: use `channels.imessage.accounts` with per-account config and optional `name`. See [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) for the shared pattern. Don't commit `~/.bot/bot.json` (it often contains tokens).
...
If you explicitly configure a `chat_id` under `channels.imessage.groups`, Bot treats that thread as a “group” for:
