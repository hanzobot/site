...
```bash
bot plugins install @bot/msteams
```

Local checkout (when running from a git repo):
```bash
bot plugins install ./extensions/msteams
```
...
1. **Install the Microsoft Teams plugin**
   - From npm: `bot plugins install @bot/msteams`
   - From a local checkout: `bot plugins install ./extensions/msteams`
...
- CLI: `bot message poll --channel msteams --target conversation:<id> ...`
...
**CLI:**
```bash
bot message send --channel msteams \
...
bot message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
bot message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
bot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
bot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
