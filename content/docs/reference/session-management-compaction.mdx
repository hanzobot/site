...
This document explains how Bot manages sessions end-to-end:
...
Bot is designed around a single **Gateway process** that owns session state.
...
Bot persists sessions in two layers:
...
Bot resolves these via `src/config/sessions.ts`.
...
Bot intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Bot consumes the events, but Pi decides when to compact).
...
Bot also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Bot bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Bot leaves it alone.
...
Bot supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:
- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Bot strips/suppresses this in the delivery layer.

As of `2026.1.10`, Bot also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Bot uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Bot’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `bot status`.
