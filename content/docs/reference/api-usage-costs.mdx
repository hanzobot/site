...
- `bot status --usage` and `bot channels list` show provider **usage windows**
...
### 8) Model scan / probe
`bot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
