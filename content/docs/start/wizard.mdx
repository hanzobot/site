...
description: "CLI onboarding wizard: guided setup for gateway, workspace, channels, and skills"
---

# Onboarding Wizard (CLI)

The onboarding wizard is the **recommended** way to set up Bot on macOS,
...
(`web_fetch` works without a key). Easiest path: `bot configure --section web`
...
1) **Existing config detection**
   ...
   - If the config is invalid or contains legacy keys, the wizard stops and asks
     you to run `bot doctor` before continuing.
...
5) **Channels**
  ...
  - DM security: default is pairing. First DM sends a code; approve via `bot pairing approve <channel> <code>` or use allowlists.
...
7) **Health check**
   - Starts the Gateway (if needed) and runs `bot health`.
   - Tip: `bot status --deep` adds gateway health probes to status output (requires a reachable gateway).
...
Use `bot agents add <name>` to create a separate agent with its own workspace,
...
Notes:
- Default workspaces follow `~/bot-<agentId>`.
...
`bot agents add` writes `agents.list[]` and optional `bindings`.
