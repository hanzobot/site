...
description: "First-run onboarding flow for Bot (macOS app)"
---
...
- Writes credentials to `~/.bot/credentials/oauth.json`
...
The app can install the global `bot` CLI via npm/pnpm so terminal
...
On the first agent run, Bot bootstraps a workspace (default `~/bot`):
...
```bash
bot webhooks gmail setup --account you@gmail.com
```
