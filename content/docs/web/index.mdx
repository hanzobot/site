...
{
  gateway: {
    controlUi: { enabled: true, basePath: "/bot" } // basePath optional
  }
}
```
...
```json5
{
  gateway: {
    ...
    auth: { mode: "password" } // or BOT_GATEWAY_PASSWORD
  }
}
```
...
- Binding the Gateway to a non-loopback address **requires** auth (`gateway.auth` or `BOT_GATEWAY_TOKEN`).
