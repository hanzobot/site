---
title: "Bot on macOS VMs (Sandboxing)"
description: "Run Bot in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
---
...
1. Install Lume
2. `lume create bot --os macos --ipsw latest`
3. Complete Setup Assistant, enable Remote Login (SSH)
4. `lume run bot --no-display`
...
```bash
lume create bot --os macos --ipsw latest
```
...
```bash
ssh youruser@192.168.64.X "bot status"
```
...
```bash
lume stop bot
lume clone bot bot-golden
```

Reset anytime:

```bash
lume stop bot && lume delete bot
lume clone bot-golden bot
lume run bot --no-display
```
...
| WhatsApp QR not scanning | Ensure you're logged into the VM (not host) when running `bot channels login` |
