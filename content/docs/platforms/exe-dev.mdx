---
title: "exe.dev"
description: "Run Bot Gateway on exe.dev (VM + HTTPS proxy) for remote access"
---

# exe.dev

Goal: Bot Gateway running on an exe.dev VM, reachable from your laptop via:
...
1) Create VM → install Node 22 → install Bot  
2) Run `bot onboard --install-daemon`  
...
Tip: keep this VM **stateful**. Bot stores state under `~/.bot/` and `~/bot/`.
...
```bash
ssh -N -L 18789:127.0.0.1:18789 bot.exe.xyz
```
...
```bash
export BOT_GATEWAY_TOKEN="$(openssl rand -hex 32)"
bot gateway --bind lan --port 8080 --token "$BOT_GATEWAY_TOKEN"
```

For service runs, persist it in `~/.bot/bot.json`:
...
On Linux, Bot uses a systemd **user** service. After `--install-daemon`, verify:

```bash
systemctl --user status bot-gateway[-<profile>].service
```
