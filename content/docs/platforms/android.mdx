...
- You can run the CLI (`bot`) on the gateway machine (or via SSH).
...
2) Configure Tailscale split DNS for `bot.internal` pointing at that DNS server.
...
1) Create `~/bot/canvas/index.html` on the gateway host.

2) Navigate the node to it (LAN):

```bash
bot nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__bot__/canvas/"}'
```
