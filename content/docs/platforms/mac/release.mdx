---
title: "Bot macOS release (Sparkle)"
description: "Bot macOS release checklist (Sparkle feed, packaging, signing)"
---
...
- Notary credentials (keychain profile or API key) for `xcrun notarytool` if you want Gatekeeper-safe DMG/zip distribution.
  - We use a Keychain profile named `bot-notary`, created from App Store Connect API key env vars in your shell profile:
    - `APP_STORE_CONNECT_API_KEY_P8`, `APP_STORE_CONNECT_KEY_ID`, `APP_STORE_CONNECT_ISSUER_ID`
    - `echo "$APP_STORE_CONNECT_API_KEY_P8" | sed 's/\\n/\n/g' > /tmp/bot-notary.p8`
    - `xcrun notarytool store-credentials "bot-notary" --key /tmp/bot-notary.p8 --key-id "$APP_STORE_CONNECT_KEY_ID" --issuer "$APP_STORE_CONNECT_ISSUER_ID"`
...
ditto -c -k --sequesterRsrc --keepParent dist/Bot.app dist/Bot-2026.1.25.zip

# Optional: also build a styled DMG for humans (drag to /Applications)
scripts/create-dmg.sh dist/Bot.app dist/Bot-2026.1.25.dmg
...
ditto -c -k --keepParent apps/macos/.build/release/Bot.app.dSYM dist/Bot-2026.1.25.dSYM.zip
```

## Appcast entry
Use the release note generator so Sparkle renders formatted HTML notes:
```bash
SPARKLE_PRIVATE_KEY_FILE=/path/to/ed25519-private-key scripts/make_appcast.sh dist/Bot-2026.1.25.zip https://raw.githubusercontent.com/bot/bot/main/appcast.xml
```
...
## Publish & verify
- Upload `Bot-2026.1.25.zip` (and `Bot-2026.1.25.dSYM.zip`) to the GitHub release for tag `v2026.1.25`.
