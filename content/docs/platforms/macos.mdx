---
title: "Bot macOS Companion (menu bar + gateway broker)"
description: "Bot macOS companion app (menu bar + gateway broker)"
---

The macOS app is the **menu‑bar companion** for Bot. It owns permissions,
...
- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `bot gateway install`.
...
The app manages a per‑user LaunchAgent labeled `com.bot.gateway`
(or `com.bot.<profile>` when using `--profile`/`BOT_PROFILE`).

```bash
launchctl kickstart -k gui/$UID/com.bot.gateway
...
Tip: compare against `bot gateway discover --json` to see whether the
