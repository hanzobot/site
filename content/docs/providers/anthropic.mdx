...
description: "Use Anthropic Claude via API keys or Claude Code CLI auth in Bot"
---

Anthropic builds the **Claude** model family and provides access via an API.
In Bot you can authenticate with an API key or reuse **Claude Code CLI** credentials
...
# or non-interactive
bot onboard --anthropic-api-key "$ANTHROPIC_API_KEY"
```
...
Bot does **not** override Anthropic’s default cache TTL unless you set it.
...
- Generate the setup-token with `claude setup-token` and paste it, or run `bot models auth setup-token` on the gateway host.
...
- Bot writes `auth.profiles["anthropic:claude-cli"].mode` as `"oauth"` so the profile
...
- If the Claude CLI login lives on a different machine, use
  `bot models auth paste-token --provider anthropic` on the gateway host.

**No API key found for provider "anthropic"**
- Auth is **per agent**. New agents don’t inherit the main agent’s keys.
- Re-run onboarding for that agent, or paste a setup-token / API key on the
  gateway host, then verify with `bot models status`.

**No credentials found for profile `anthropic:default` or `anthropic:claude-cli`**
- Run `bot models status` to see which auth profile is active.
- Re-run onboarding, or paste a setup-token / API key for that profile.

**No available auth profile (all in cooldown/unavailable)**
- Check `bot models status --json` for `auth.unusableProfiles`.
