---
title: "Venice AI (Venius highlight)"
description: "Use Venice AI privacy-focused models in Bot"
---
...
```bash
bot chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Bot shows all available Venice models. Pick based on your needs:
...
```bash
bot models set venice/claude-opus-45
bot models set venice/llama-3.3-70b
```
...
Bot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
bot chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
bot chat --model venice/claude-opus-45

# Use uncensored model
bot chat --model venice/venice-uncensored

# Use vision model with image
bot chat --model venice/qwen3-vl-235b-a22b

# Use coding model
bot chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `bot models list` to see currently available models. Some models may be temporarily offline.
