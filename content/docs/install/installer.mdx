...
Bot ships two installer scripts (served from `hanzo.bot`):

- `https://hanzo.bot/install.sh` — “recommended” installer (global npm install by default; can also install from a GitHub checkout)
- `https://hanzo.bot/install-cli.sh` — non-root-friendly CLI installer (installs into a prefix with its own Node)
 - `https://hanzo.bot/install.ps1` — Windows PowerShell installer (npm by default; optional git install)

To see the current flags/behavior, run:

```bash
curl -fsSL https://hanzo.bot/install.sh | bash -s -- --help
```

Windows (PowerShell) help:

```powershell
& ([scriptblock]::Create((iwr -useb https://hanzo.bot/install.ps1))) -?
```

If the installer completes but `bot` is not found in a new terminal, it’s usually a Node/npm PATH issue. See: [Install](/install#nodejs--npm-path-sanity).
...
- Choose install method:
  - `npm` (default): `npm install -g bot@latest`
...
- If upgrading an existing install: runs `bot doctor --non-interactive` (best effort).
- For git installs: runs `bot doctor --non-interactive` after install/update (best effort).
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL https://hanzo.bot/install.sh | bash
```

### Discoverability / “git install” prompt

If you run the installer while **already inside a Bot source checkout** (detected via `package.json` + `pnpm-workspace.yaml`), it prompts:

- update and use this checkout (`git`)
- or migrate to the global npm install (`npm`)

In non-interactive contexts (no TTY / `--no-prompt`), you must pass `--install-method git|npm` (or set `BOT_INSTALL_METHOD`), otherwise the script exits with code `2`.
...
This script installs `bot` into a prefix (default: `~/.bot`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you don’t want to touch the system Node/npm.

Help:

```bash
curl -fsSL https://hanzo.bot/install-cli.sh | bash -s -- --help
```
...
```powershell
iwr -useb https://hanzo.bot/install.ps1 | iex -InstallMethod git
```

```powershell
iwr -useb https://hanzo.bot/install.ps1 | iex -InstallMethod git -GitDir "C:\\bot"
```
...
- **"bot" is not recognized**: your npm global bin folder is not on PATH. Most systems use
