...
description: "Optional Docker-based setup and onboarding for Bot"
---
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Bot on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:
- Containerized Gateway (full Bot in Docker)
...
```bash
docker build -t bot:local -f Dockerfile .
docker compose run --rm bot-cli onboard
...
`bot-gateway` and `bot-cli` by generating `docker-compose.extra.yml`.
...
Bot auto-recreates containers when `setupCommand` (or docker config) changes
unless the container was **recently used** (within ~5 minutes). Hot containers
log a warning with the exact `bot sandbox recreate ...` command.
...
This builds `bot-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `bot-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: { defaults: { sandbox: { docker: { image: "bot-sandbox-common:bookworm-slim" } } } }
}
```
...
This builds `bot-sandbox-browser:bookworm-slim` using
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-bot-browser" } }
    }
  }
}
```
...
```bash
docker build -t my-bot-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-bot-sbx" } }
    }
  }
}
```
...
- Custom tools not found: Bot runs commands with `sh -lc` (login shell), which
