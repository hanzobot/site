...
“Context” is **everything Bot sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:
- **System prompt** (Bot-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Bot builds the system prompt

The system prompt is **Bot-owned** and rebuilt each run. It includes:
...
By default, Bot injects a fixed set of workspace files (if present):
- `AGENTS.md`
- `SOUL.md`
- `TOOLS.md`
- `IDENTITY.md`
