---
title: "System Prompt"
description: "What the Bot system prompt contains and how it is assembled"
---

Bot builds a custom system prompt for every agent run. The prompt is **Bot-owned** and does not use the p-coding-agent default prompt.
...
Skills (https://skills.com) for skills discovery. The prompt instructs the model to consult local docs first
for Bot behavior, commands, configuration, or architecture, and to run
`bot status` itself when possible (asking the user only when it lacks access).
