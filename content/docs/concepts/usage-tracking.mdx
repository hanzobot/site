...
- `/usage cost` in chats: local cost summary aggregated from Bot session logs.
- CLI: `bot status --usage` prints a full per-provider breakdown.
- CLI: `bot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
