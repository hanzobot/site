---
title: "OAuth"
description: "OAuth in Bot: token exchange, storage, CLI sync, and multi-account patterns"
---

Bot supports “subscription auth” via OAuth for providers that offer it (notably **Anthropic (Claude Pro/Max)** and **OpenAI Codex (ChatGPT OAuth)**). This page explains:
...
Practical symptom:
- you log in via Bot *and* via Claude Code / Codex CLI → one of them randomly gets “logged out” later
...
- Runtime cache (managed automatically; don’t edit): `~/.bot/agents/<agentId>/agent/auth.json`
...
If you already signed in with the external CLIs *on the gateway host*, Bot can reuse those tokens without starting a separate OAuth flow:
...
Sync happens when Bot loads the auth store (so it stays up-to-date when the CLIs refresh tokens).
On macOS, the first read may trigger a Keychain prompt; run `bot models status`
...
The wizard path is `bot onboard` → auth choice `oauth` (Anthropic).
...
Wizard path is `bot onboard` → auth choice `openai-codex` (or `codex-cli` to reuse an existing Codex CLI login).
...
When Bot refreshes an Anthropic OAuth token (profile `anthropic:claude-cli`), it **writes the new credentials back** to Claude Code's storage:
...
1. Bot refreshes the token → gets new access token
...
- `bot channels list --json` (shows `auth[]`)
