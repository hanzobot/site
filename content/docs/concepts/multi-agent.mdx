...
- Workspace defaults to `~/bot` (or `~/bot-<profile>` when `BOT_PROFILE` is set).
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/bot-alex" },
      { id: "mia", workspace: "~/bot-mia" }
    ]
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "chat", name: "Everyday", workspace: "~/bot-chat", model: "anthropic/claude-sonnet-4-5" },
      { id: "opus", name: "Deep Work", workspace: "~/bot-opus", model: "anthropic/claude-opus-4-5" }
    ]
  },
  bindings: [
