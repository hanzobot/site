---
title: "Agent Runtime 🤖"
description: "Agent runtime (embedded p-mono), workspace contract, and session bootstrap"
---

Bot runs a single embedded agent runtime derived from **p-mono**.
...
Recommended: use `bot setup` to create `~/.bot/bot.json` if missing and initialize the workspace files.
...
If a file is missing, Bot injects a single “missing file” marker line (and `bot setup` will create a safe default template).
...
Bot reuses pieces of the p-mono codebase (models/tools), but **session management, discovery, and tool wiring are Bot-owned**.
