...
description: "Chrome extension: let Bot drive your existing Chrome tab"
---

# Chrome extension (browser relay)

The Bot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate bot-managed Chrome profile.
...
After upgrading Bot:
- Re-run `bot browser extension install` to refresh the installed files under your Bot state directory.
...
If you see `!`:
- Make sure the Gateway is running locally (default setup), or run `bot browser serve` on this machine (remote gateway setup).
- Open the extension Options page; it shows whether the relay is reachable.

## Do I need `bot browser serve`?
...
you typically **do not** need `bot browser serve`.

The Gateway’s built-in browser control server will start on `http://127.0.0.1:18791/` and Bot will
auto-start the local relay server on `http://127.0.0.1:18792/`.

### Remote Gateway (Gateway runs elsewhere) — **yes**

If your Gateway runs on another machine, run `bot browser serve` on the machine that runs Chrome
...
`bot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `bot browser extension install` first to copy the extension to a stable location under your Bot state directory.
...
- **This is not isolated** like the dedicated bot-managed profile.
