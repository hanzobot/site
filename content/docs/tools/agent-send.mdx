---
title: "`bot agent` (direct agent runs)"
description: "Direct `bot agent` CLI runs (with optional delivery)"
---

`bot agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `bot message --target`).
...
```bash
bot agent --to +15555550123 --message "status update"
bot agent --agent ops --message "Summarize logs"
...
bot agent --to +15555550123 --message "Summon reply" --deliver
