---
title: "Tools (Bot)"
description: "Agent tool surface for Bot (browser, canvas, nodes, message, cron) replacing legacy `bot-*` skills"
---

# Tools (Bot)

Bot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `bot-*` skills: the tools are typed, no shelling,
...
- [Pipeline](/tools/pipeline): typed workflow runtime with resumable approvals (requires the Pipeline CLI on the gateway host).
...
- `host=node` can target a macOS companion app or a headless node host (`bot node run`).
...
Notes:
- Requires a Brave API key (recommended: `bot configure --section web`, or set `BRAVE_API_KEY`).
...
### `browser`
Control the dedicated bot browser.
...
- Quick smoke: `bot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `bot gateway` restart in-place)
