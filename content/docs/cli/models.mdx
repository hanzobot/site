---
title: "`bot models`"
description: "CLI reference for `bot models` (status/list/set/scan, aliases, fallbacks, auth)"
---
...
`bot models status` shows the resolved default/fallbacks plus an auth overview.
...
bot models auth setup-token
bot models auth paste-token
```
`models auth login` runs a provider plugin’s auth flow (OAuth/API key). Use
`bot plugins list` to see which providers are installed.
