...
- Replaces the legacy `gateway` command. CLI entry point: `bot gateway`.
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__bot__/canvas/` from `~/bot/canvas`. Disable with `canvasHost.enabled=false` or `BOT_SKIP_CANVAS_HOST=1`.
...
- Gateway auth: set `gateway.auth.mode=token` + `gateway.auth.token` (or pass `--token <value>` / `BOT_GATEWAY_TOKEN`) to require clients to send `connect.params.auth.token`.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `BOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `com.bot.<profile>`
- Linux: `bot-gateway-<profile>.service`
...
- `agents.defaults.workspace` default becomes `~/bot-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):
- Base port = `gateway.port` (or `BOT_GATEWAY_PORT` / `--port`)
- `browser.controlUrl port = base + 2` (or `BOT_BROWSER_CONTROL_URL` / config override)
- `canvasHost.port = base + 4` (or `BOT_CANVAS_HOST_PORT` / config override)
...
```bash
bot --profile main gateway install
bot --profile rescue gateway install
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `bot health --json`).
...
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `bot gateway install` writes `~/Library/LaunchAgents/com.bot.gateway.plist`
    (or `com.bot.<profile>.plist`).
  - `bot doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Bot profile services.
  ...
  - Cleanup: `bot gateway uninstall` (current service) and `bot doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `bot gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- Bot.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `com.bot.gateway` (or `com.bot.<profile>`).
- To stop it cleanly, use `bot gateway stop` (or `launchctl bootout gui/$UID/com.bot.gateway`).
- To restart, use `bot gateway restart` (or `launchctl kickstart -k gui/$UID/com.bot.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `bot gateway install` first.
  - Replace the label with `com.bot.<profile>` when running a named profile.

## Supervision (systemd user unit)
Bot installs a **systemd user service** by default on Linux/WSL2. We
...
`bot gateway install` writes the user unit. `bot doctor` audits the
...
Create `/etc/systemd/system/bot-gateway[-<profile>].service` (copy the unit above,
...
## CLI helpers
- `bot gateway health|status` — request health/status over the Gateway WS.
- `bot message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `bot agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `bot gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `bot gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `bot gateway` and the legacy TCP control port.
