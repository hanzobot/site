---
title: "Configuration 🔧"
description: "All configuration options for ~/.bot/bot.json with examples"
---

Bot reads an optional **JSON5** config from `~/.bot/bot.json` (comments + trailing commas allowed).

If the file is missing, Bot uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/bot`). You usually only need a config to:
...
Bot only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `bot doctor`, `bot logs`, `bot health`, `bot status`, `bot service`, `bot help`).
- Run `bot doctor` to see the exact issues.
- Run `bot doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `bot config set`. Keep a backup of `~/.bot/bot.json`.
...
{
  agents: {
    defaults: { workspace: "~/bot" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@bot", "reisponde"] }
      }
    ]
  },
  channels: {
...
{ "$include": "/etc/bot/base.json5" } // absolute
...
Bot reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Bot runs your login shell and imports only the missing expected keys (never overrides).
...
Bot stores **per-agent** auth profiles (OAuth + API keys) in:
...
- `~/.bot/agent/*` (migrated by `bot doctor` into `~/.bot/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `BOT_OAUTH_DIR`
- Agent dir (default agent root override): `BOT_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Bot imports `oauth.json` entries into `auth-profiles.json`.

Bot also auto-syncs OAuth tokens from external CLIs into `auth-profiles.json` (when present on the gateway host):
...
credential is a setup-token. Bot auto-migrates older configs that used
`mode: "token"`.
...
If set, Bot derives defaults (only when you haven’t set them explicitly):
...
- If you want a stable path, set `logging.file` to `/tmp/bot/bot.log`.
...
- The legacy single-account Baileys auth dir is migrated by `bot doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@bot", "bot", "bot"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/bot-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/bot-home" },
      { id: "work", workspace: "~/bot-work" }
    ]
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `bot.json`).
...
Bot starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["bot-dm"]          // optional group DM allowlist
      },
      guilds: {
...
Bot starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `bot plugins install @bot/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Bot starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Bot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Bot
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/bot" } }
}
```
...
When a file exceeds this limit, Bot logs a warning and injects a truncated
...
message envelopes). If unset, Bot uses the host timezone at runtime.
...
`[bot]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[bot]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[bot]"`, Bot will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Bot generates audio
...
Bot also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Bot currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `bot configure --section web`, or use `BRAVE_API_KEY` env var)
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Bot uses
...
- Shortcut: `bot onboard --auth-choice opencode-zen`.
...
Shortcut: `bot onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `bot onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `bot onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `bot onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `bot onboard --auth-choice minimax-api`.
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Bot stays in idle-only mode for backward compatibility.
...
### `browser` (bot-managed browser)

Bot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for bot and expose a small loopback control server.
...
- Note: the control server is started by the running gateway (Bot.app menubar, or `bot gateway`).
...
- When a connection comes from one of these IPs, Bot uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `bot gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `BOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
- Non-loopback binds (`lan`/`tailnet`/`auto`) require auth. Use `gateway.auth.token` (or `BOT_GATEWAY_TOKEN`).
...
- `gateway.auth.password` can be set here, or via `BOT_GATEWAY_PASSWORD` (recommended).
...
macOS app behavior:
- Bot.app watches `~/.bot/bot.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
- `BOT_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `bot --dev …` → uses `~/.bot-dev` + shifts ports from base `19001`
- `bot --profile <name> …` → uses `~/.bot-<name>` (port via config/env/flags)
...
Gmail helper config (used by `bot webhooks gmail setup` / `run`):
...
- Set `BOT_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Bot defaults `serve.path` to `/` so
...
Default root: `~/bot/canvas`  
Default port: `18793` (chosen to avoid the bot browser CDP port `18792`)  
The server listens on the **gateway bind host** (LAN or Tailnet) so nodes can reach it.

The server:
- serves files under `canvasHost.root`
- injects a tiny live-reload client into served HTML
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__bot/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__bot__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_bot-bridge._tcp` under `~/.bot/dns/` using the standard discovery domain `bot.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `bot.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `bot.internal` via that server
