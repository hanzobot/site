...
description: "Model authentication: OAuth, API keys, and Claude Code token reuse"
---

Bot supports OAuth and API keys for model providers. For Anthropic
accounts, we recommend using an **API key**. Bot can also reuse Claude Code
...
2) Put it on the **gateway host** (the machine running `bot gateway`).
...
API keys for daemon use: `bot onboard`.
...
Bot config sets `auth.profiles["anthropic:claude-cli"].mode` to `"oauth"` so
...
Alternative: run the wrapper (also updates Bot config):
...
bot models auth paste-token --provider openrouter
```

Automation-friendly check (exit `1` when expired/missing, `2` when expiring):

```bash
bot models status --check
```

Optional ops scripts (systemd/Termux) are documented here:
[/automation/auth-monitoring](/automation/auth-monitoring)

`bot models status` loads Claude Code credentials into Bot’s
`auth-profiles.json` and shows expiry (warns within 24h by default).
`bot doctor` also performs the sync when it runs.
...
1. **Claude Code** stores credentials in `~/.claude/.credentials.json` (or
   Keychain on macOS).
2. **Bot** syncs those into
   `~/.bot/agents/<agentId>/agent/auth-profiles.json` when the auth store is
   loaded.
...
Run `bot models status` to confirm which profile is expiring. If the profile
