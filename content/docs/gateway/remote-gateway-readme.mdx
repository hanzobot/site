---
title: "Running Bot.app with a Remote Gateway"
description: "SSH tunnel setup for Bot.app connecting to a remote gateway"
---
...
│                        Client Machine                          │
│                                                              │
│  Bot.app ──► ws://127.0.0.1:18789 (local port)           │
│                     │                                        │
│                     ▼                                        │
│  SSH Tunnel ────────────────────────────────────────────────│
│                     │                                        │
...
Save this as `~/Library/LaunchAgents/com.bot.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>com.bot.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/com.bot.ssh-tunnel.plist
```
...
```bash
launchctl kickstart -k gui/$UID/com.bot.ssh-tunnel
```

**Stop the tunnel:**

```bash
launchctl bootout gui/$UID/com.bot.ssh-tunnel
```
