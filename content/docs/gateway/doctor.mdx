...
`bot doctor` is the repair + migration tool for Bot. It fixes stale
...
- Extra workspace dir detection (`~/bot`).
...
offers to remove them and install the Bot service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Bot gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `bot doctor --generate-gateway-token` to force token
...
Notes:
- `bot doctor` prompts before rewriting supervisor config.
- `bot doctor --yes` accepts the default repair prompts.
- `bot doctor --repair` applies recommended fixes without prompts.
- `bot doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `bot gateway install --force`.
