...
description: "Run multiple Bot Gateways on one host (isolation, ports, and profiles)"
---
...
```bash
# main
bot --profile main setup
bot --profile main gateway --port 18789

# rescue
bot --profile rescue setup
bot --profile rescue gateway --port 19001
```
...
```bash
bot --profile main status
bot --profile rescue status
bot --profile rescue browser status
```
