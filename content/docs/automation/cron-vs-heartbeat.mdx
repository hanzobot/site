...
bot cron add --name "Morning brief" --cron "0 7 * * *" --session isolated --message "..." --deliver

# Weekly project review on Mondays at 9am
bot cron add --name "Weekly review" --cron "0 9 * * 1" --session isolated --message "..." --model opus

# One-shot reminder
bot cron add --name "Call back" --at "2h" --session main --system-event "Call back the client" --wake now
```
...
- **Pipeline** defines *what steps* happen once the run starts.
...
- The tool is an **optional plugin**; you must allowlist `pipeline` in `tools.allow`.
- If you pass `pipelinePath`, it must be an **absolute path**.
