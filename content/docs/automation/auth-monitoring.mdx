...
Bot exposes OAuth expiry health via `bot models status`. Use that for
...
- `scripts/claude-auth-status.sh` now uses `bot models status --json` as the
  source of truth (falling back to direct file reads if the CLI is unavailable),
  so keep `bot` on `PATH` for timers.
- `scripts/auth-monitor.sh`: cron/systemd timer target; sends alerts (ntfy or phone).
- `scripts/systemd/bot-auth-monitor.{service,timer}`: systemd user timer.
